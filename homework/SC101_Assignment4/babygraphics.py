...
def draw_names(canvas, name_data, lookup_names):
    ...
    for i in range(len(lookup_names)):
        ...
        for j in range(len(YEARS)-1):
            ...
            y1 = GRAPH_MARGIN_SIZE+(rank-1)/(MAX_RANK-1)*(CANVAS_HEIGHT-2*GRAPH_MARGIN_SIZE)
            ...
            y2 = GRAPH_MARGIN_SIZE+(next_rank-1)/(MAX_RANK-1)*(CANVAS_HEIGHT-2*GRAPH_MARGIN_SIZE)

            canvas.create_line(x1, y1, x2, y2, width=LINE_WIDTH, fill=color)
            if str(YEARS[j]) in name_data[lookup_names[i]]:
                canvas.create_text(x1+TEXT_DX, y1, text=lookup_names[i]+" "+str(rank), fill=color, anchor=tkinter.SW)
            else:
                canvas.create_text(x1+TEXT_DX, y1, text=lookup_names[i] + " *", fill=color, anchor=tkinter.SW)
        if str(YEARS[len(YEARS)-1]) in name_data[lookup_names[i]]:
            canvas.create_text(x2+TEXT_DX, y2, text=lookup_names[i]+" "+str(next_rank), fill=color, anchor=tkinter.SW)
        else:
            canvas.create_text(x2+TEXT_DX, y2, text=lookup_names[i] + " *", fill=color, anchor=tkinter.SW)
